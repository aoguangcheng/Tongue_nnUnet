...
if __name__ == "__main__":
    ...
    dataset_name = 'Dataset099_Tongue'
    source = "../dataset/TongeImageDataset"
    ...
    with multiprocessing.get_context("spawn").Pool(8) as p:
        ...
        valid_ids = subfiles(join(train_source, 'origin_Image'), join=False, suffix='png')
        ...
        for v in valid_ids:
            r.append(
                p.starmap_async(
                    ...
                    ((
                         join(train_source, 'origin_Image', v),
                         join(train_source, 'origin_GT', v),
        ...
        # test set
        valid_ids = subfiles(join(test_source, 'origin_Image'), join=False, suffix='png')
        for v in valid_ids:
            r.append(
                p.starmap_async(
                    load_and_covnert_case,
                    ((
                         join(test_source, 'origin_Image', v),
                         join(test_source, 'origin_GT', v),
